...
import { unitCostFromLine, weightedAverage } from './costing';
...
export function postReceipt(input: NewReceipt): number {
	...
	const run = db.transaction((): number => {
		...
		const receiptId = db
			.prepare(
				`INSERT INTO receipts (vendor_id, ref_number, purchased_at, subtotal, tax, shipping, total, allocate_extras, notes)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
			)
			.run(
				input.vendor_id,
